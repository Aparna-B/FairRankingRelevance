...
if __name__ == '__main__':
	...
	for dist in ['normal','pareto']:
		...
		df=pd.read_csv('R{}_v3.csv'.format(dist))
		...
		df_train.to_csv('synthetic_{}_train.csv'.format(dist),index=False)
		# df_test['U']=np.array(df_test['U'].values>upper_quartile,dtype=np.int8)
		df_test.to_csv('synthetic_test_{}.csv'.format(dist),index=False)
		...
		get_all_txt_rows(df_train,'U','data/train_{}/train_{}.txt'.format(dist,dist),upper_quartile)
		get_all_txt_rows(df_val,'U','data/val_{}/val_{}.txt'.format(dist,dist),upper_quartile)
		get_all_txt_rows(df_test,'U','data/test_{}/test_{}.txt'.format(dist,dist),upper_quartile)
