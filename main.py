...
"""
#April 9th, 4:28: modifying "output_replicate" to "output_replicated"
...
parser.add_argument("--model_dir", type=str, default="./model_outs/", help="The directory for model and "
                                                                              "intermediate outputs.")
parser.add_argument("--output_dir", type=str, default="./model_outs/", help="The directory to output results.")
...
parser.add_argument("--max_train_iteration", type=int, default=500,
...
parser.add_argument("--start_saving_iteration", type=int, default=0,
                    help="The minimum number of iterations before starting to test and save models. "
                         "(0: no limit).")
parser.add_argument("--steps_per_checkpoint", type=int, default=10,
...
def create_model(exp_settings, data_set):
    ...
    try:
        checkpoint_path = os.path.join(args.model_dir, "{}_{}_{}.ckpt".format(exp_settings['learning_algorithm'],
                                                                args.dsname,
                                                                args.seed))
        ckpt = torch.load(checkpoint_path)
        print("Note: checkpointing enabled for %s" % checkpoint_path)
        assert not args.train_flag
        if args.train_flag:
            np.save('{}_{}.npy'.format(args.dsname,args.seed),np.array([0]))
        model.model.load_state_dict(ckpt)
        model.model.eval()

    except FileNotFoundError:
        # this means model is training, so checkpoint for this model
        # is not available.
        assert args.train_flag 
    return model
...
def train(exp_settings):
    ...
    train_writer = torch.utils.tensorboard.SummaryWriter(log_dir=args.model_dir + '/train_{}_{}_log'.format(args.dsname,
        args.seed))
    valid_writer = torch.utils.tensorboard.SummaryWriter(log_dir=args.model_dir + '/valid_{}_{}_log'.format(args.dsname,
        args.seed))
    test_writer = None
    if args.test_while_train:
        test_writer = torch.utils.tensorboard.SummaryWriter(log_dir=args.model_dir + '/test_{}_{}_log'.format(args.dsname,
        args.seed))
    ...
    while True:
        ...
        if current_step % args.steps_per_checkpoint == 0:
            ...
            # Validate model
            def validate_model(data_set, data_input_feed, save_file=False):
                it = 0
                count_batch = 0.0
                summary_list = []
                batch_size_list = []
                ...
                while it < len(data_set.initial_list):
                    ...
                    _, _, summary, output_logits = model.validation(input_feed, return_unpadded=True)
                    ...
                    count_batch += 1.0
                    all_logits.extend(output_logits)
                if save_file:
                    if 'mismatched' in args.output_dir:
                        out_file='./csv_outs/output_replicated/val_'+str(current_step)+'_mismatched_'+args.dsname+'_{}.csv'.format(seed)
                        all_scores_out_file='./csv_outs/output_replicated/pred_rel/val_'+str(current_step)+'_mismatched_'+args.dsname+'_{}.csv'.format(seed)
                    else:
                        out_file='./csv_outs/output_replicated/val_'+str(current_step)+'_matched_'+args.dsname+'_{}.csv'.format(seed)
                        all_scores_out_file='./csv_outs/output_replicated/pred_rel/val_'+str(current_step)+'_matched_'+args.dsname+'_{}.csv'.format(seed)

                    rerank_scores_numpy=[score.data.numpy() for score in all_logits]
                    # assert len(rerank_scores_numpy)==5000
                    pd.DataFrame(rerank_scores_numpy).to_csv(all_scores_out_file, index=False)


                return ultra.utils.merge_Summary(summary_list, batch_size_list)
                # return summary_list

            valid_summary = validate_model(valid_set, valid_input_feed, save_file=True)
                        ...
                        if current_step >= args.start_saving_iteration:
                            if best_perf == None or best_perf < value:
                                checkpoint_path = os.path.join(args.model_dir,
                                                               "{}_{}_{}.ckpt".format(exp_settings['learning_algorithm'],
                                                                args.dsname,
                                                                args.seed))
                                torch.save(model.model.state_dict(), checkpoint_path)
                                best_perf = value
                                best_iter = current_step
                                np.save('best_iter.npy',np.array(best_iter))
                                break
...
def test(exp_settings):
    ...
    df_output = pd.DataFrame(test_summary, index=[0])

    os.makedirs('./csv_outs',exist_ok=True)
    os.makedirs('./csv_outs/pred_rel',exist_ok=True)

    if 'mismatched' in args.output_dir:
        out_file='./csv_outs/output_replicated/mismatched_'+args.test_data_prefix+'_{}.csv'.format(seed)
        all_scores_out_file='./csv_outs/output_replicated/pred_rel/mismatched_'+args.test_data_prefix+'_{}.csv'.format(seed)
    else:
        out_file='./csv_outs/output_replicated/matched_'+args.test_data_prefix+'_{}.csv'.format(seed)
        all_scores_out_file='./csv_outs/output_replicated/pred_rel/matched_'+args.test_data_prefix+'_{}.csv'.format(seed)
...
def train_test_comb(_):
    exp_settings = json.load(open(args.setting_file))
    print(args.max_train_iteration)
    train(exp_settings)
...
if __name__ == "__main__":
    argv = sys.argv
    #NB: Commenting out: main(argv)
    train_test_comb(argv)
